...
export class PackagesController {
    private cnfg = config();
    constructor(@Inject(CACHE_MANAGER) private cacheManager: Cache, private readonly packagesService: PackagesService, private readonly s3Service: S3BucketService) {}
    ...
    async getPackages(@Query('hasGallery') hasGallery?: boolean, @Query('hasBanner') hasBanner?: boolean, @Query('pageNumber') pageNumber?: number, @Query('pageSize') pageSize?: number): Promise<TResponseData> {
        ...
        const imaged_packages = await Promise.all(packages.records.map(async (data) => {
            const { id, package_number, package_slug, package_title, thumbnail, package_banner_image, package_details, price, discount } = data;
                ...
                if (hasGallery) packageInfo.gallery = await Promise.all(galleryImageIds.map(async gId => {
                    const galleryImage = await this.s3Service.getImage(data[gId]);
                    return galleryImage;
                }));
                if (hasBanner) packageInfo.package_banner_image = await this.s3Service.getImage(package_banner_image);
                return packageInfo;
            }
        ));
    ...
    async getPackageBySlug(@Query('package_slug') slug: string): Promise<TResponseData> {
